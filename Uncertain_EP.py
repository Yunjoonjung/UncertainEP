...
#-----------------------------------------------------------------------------------------------------------------------------------#     
    def EP_iteration(self, SA_quantified_matrix):  # This is used only for sensitivity analysis
        ...
        if self.number_of_samples < 10: # one digit case # => # of SA samples 
            idf_instance_name = idf_instance_name[:-2]
        elif 10 <= self.number_of_samples < 100: # two digit cases
            idf_instance_name = idf_instance_name[:-3]
        elif 100 <= self.number_of_samples < 1000: # three digit cases
            idf_instance_name = idf_instance_name[:-4]            
        elif 1000 <= self.number_of_samples < 10000: # four digit cases
            idf_instance_name = idf_instance_name[:-5]
        elif 10000 <= self.number_of_samples < 100000: # five digit cases
            idf_instance_name = idf_instance_name[:-6] 
...
#-----------------------------------------------------------------------------------------------------------------------------------#
    def SA(self, number_of_SA_samples=1000):
        ...
        print("\nResult of Morris Method is {}".format(Si))
        uncertain_input.save('Uncertain_EP_Input.xlsx') # close Excel
...
#---------------------------------------------------------------#
        ...
        if self.number_of_samples < 10: # one digit case # => # of UA samples 
            idf_instance_name = idf_instance_name[:-2]
        elif 10 <= self.number_of_samples < 100: # two digit cases
            idf_instance_name = idf_instance_name[:-3]
        elif 100 <= self.number_of_samples < 1000: # three digit cases
